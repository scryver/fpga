...
from myhdl import Signal, intbv, modbv, always, always_comb, concat
...
def AES3_TX(audio_ch1, cs1, valid1, user1, audio_ch2, cs2, valid2, user2,
            frame0, ce_word, ce_bit, ce_bp, sdata, clk, rst):
    frame0_reg = Signal(False)
    inreg1, inreg2 = [Signal(intbv(0)[27:]) for _ in range(2)]
    sr = Signal(intbv(0)[28:])
    parity1, parity2 = [Signal(False) for _ in range(2)]
    out_xz, out_y, out_ch1, out_ch2 = [Signal(False) for _ in range(4)]
    set_out_xz, set_out_y, set_out_ch1, set_out_ch2 = [Signal(False) for _ in range(4)]
    seq = Signal(intbv(0)[37:])
    pre_sr = Signal(intbv(0)[8:])
    last_state = Signal(False)

    dout, b0, b1, txd, state = [Signal(False) for _ in range(5)]
    ...
    return state_logic, input_reg, audio_shift_reg, parity_gen, sequencer, output_setter, output_ch1, output_ch2, \
           output_xz, output_y, preamble_logic, output_mux, biphase_comb_logic_0, biphase_comb_logic_1, \
           last_state_logic, sdata_logic
...
def AES3_RX_DEMUXED(din, audio1, valid1, user1, cs1, out_en, audio2, valid2, user2, cs2,
    ...
    recdata, recdata_valid = [Signal(False) for _ in range(2)]
    framer_out = Signal(intbv(0)[8:])
    framer_valid = Signal(False)
    x_preamble, y_preamble, z_preamble = [Signal(False) for _ in range(3)]
    channel1, channel2, valid, user_data, cs = [Signal(False) for _ in range(5)]
    audio, audio1_hold = [Signal(intbv(0)[24:]) for _ in range(2)]
    locked_timeout = Signal(intbv(0)[LCK_BITS:])
    valid1_hold, user1_hold, cs1_hold = [Signal(False) for _ in range(3)]
    parity_error_int = Signal(False)
    frames_int = Signal(intbv(0)[8:])

    aes_dru = AES_Dru(din, recdata, recdata_valid, clk, rst)
    aes_framer = AES_Framer(recdata, recdata_valid, framer_out, framer_valid, x_preamble, y_preamble, z_preamble, clk,
                            rst)
    aes_rx_formatter = AES_RX_Formatter(framer_out, framer_valid, x_preamble, y_preamble, z_preamble,
                                        channel1, channel2, audio, valid, parity_error_int, user_data,
                                        cs, frames_int, clk, rst)
    ...
    return aes_dru, aes_framer, aes_rx_formatter, lock_timeout_logic, lock_logic, demux_registers, \
           output_regs, output_enable_logic, parity_error_pass
...
def AES_Dru(din, dout, dout_valid, clk, rst):
    ...
    inffs = Signal(intbv(0)[3:])  # Edge detection FFs
    edge_detect = Signal(False)  # Edge detection signal
    min_cntr, min_capture, min_hold = [Signal(modbv(0)[MIN_VAL_WIDTH:]) for _ in range(3)]  # clocks between edges, min capture reg,
    # min value hold register
    update_min, new_min = [Signal(False) for _ in range(2)]  # Update tmin capture & hold regs, symlen_cntr < min_campture
    update_cntr = Signal(modbv(0)[UPDATE_CNTR_WIDTH:])  # Update period counter
    sample_cntr = Signal(modbv(0)[MIN_VAL_WIDTH:])  # Finds sample points
    sample_now = Signal(False)  # Sample when asserted
    ...
    return ffs, edge_detection, min_counter, min_capture_reg, new_minimum, min_hold_reg, update_period_counter, \
           update_minimum, sample_counter, sample_it_now, output_data, output_valid


def AES_Framer(din, din_valid, dout, dout_valid, x_preamble, y_preamble, z_preamble, clk, rst):
    """
    This module accepts the serial bit stream from the AES data recovery unit with the accompanying
    data valid bit. It identifies the preamble words and uses them to align the data. It does the
    bi-phase decoding and outputs 8-bit recovered data words with a data valid bit and X, Y, and Z
    preamble indicators.
    ...
    in_sr = Signal(intbv(0)[9:])
    deser = Signal(intbv(0)[8:])
    deser_ce = Signal(False)
    predet_in = Signal(intbv(0)[8:])
    x_detect, y_detect, z_detect, pre_detect = [Signal(False) for _ in range(4)]
    fixed_match, dout_ld, state = [Signal(False) for _ in range(3)]
    bitcntr = Signal(modbv(0)[3:])
    int_xyz = Signal(intbv(0)[3:])
    ...
    def fixedmatch_input():
        fixed_match.next = predet_in[0] and predet_in[1] and predet_in[2] and not predet_in[3] and not predet_in[7]

    @always_comb
    def preamble_detect():
        x_detect.next = True if fixed_match and predet_in[7:4] == 0b100 else False
        y_detect.next = True if fixed_match and predet_in[7:4] == 0b010 else False
        z_detect.next = True if fixed_match and predet_in[7:4] == 0b001 else False

        pre_detect.next = True if fixed_match and (predet_in[7:4] == 0b001 or predet_in[7:4] == 0b010 or
    ...
    def framer_control_comb():
        ...
        dout_ld.next = True if not state and din_valid and bitcntr == 0b111 else False
    ...
    return input_shift_reg, predetect_input, fixedmatch_input, preamble_detect, deserialization, output_register, \
           framer_state_logic, framer_control_logic, framer_control_comb, delay_preamble_reg, delay_preambles, \
           dout_valid_gen
...
def AES_RX_Formatter(din, din_valid, x_preamble, y_preamble, z_preamble,
    ...
    in_reg = Signal(intbv(0)[28:])
    sub = Signal(False)
    byte_count = Signal(intbv(0)[4:])
    preamble_in = Signal(False)
    ce, ld_out = [Signal(False) for _ in range(2)]
    frames_int = Signal(modbv(0)[8:])
    ...
    return preambled, byte_counter, input_register, subframe_tracker, frame_counter, frame_comb, output_clk,\
           audio_output, channel_indicator, valid_data, user_data, channel_status, parity_error_detection, \
           output_clk_enable


def convert_tx():
    cs1, valid1, user1, cs2, valid2, user2, frame0, ce_word, ce_bit, ce_bp, sdata, clk, rst = [Signal(False) for _ in range(13)]
    audio_ch1, audio_ch2 = [Signal(intbv(0)[24:]) for _ in range(2)]

    toVHDL(AES3_TX, audio_ch1, cs1, valid1, user1, audio_ch2, cs2, valid2, user2,
           frame0, ce_word, ce_bit, ce_bp, sdata, clk, rst)


def convert_rx():
    din, valid1, user1, cs1, out_en, valid2, user2, cs2, parity_error, frame0, locked, clk, rst = [Signal(False) for _ in range(13)]
    audio1, audio2 = [Signal(intbv(0)[24:]) for _ in range(2)]
    frames = Signal(intbv(0)[8:])

    toVHDL(AES3_RX_DEMUXED, din, audio1, valid1, user1, cs1, out_en, audio2, valid2, user2, cs2,
           parity_error, frames, frame0, locked, clk, rst)
